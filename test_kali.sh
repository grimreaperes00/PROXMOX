...
echo "========================================="
echo "[1/10] 檢查並自動分配可用 VM ID ..."
echo "========================================="
while qm status "$vm_id" &>/dev/null; do
  echo "VM ID $vm_id 已存在，嘗試使用下一個 ID ..."
  vm_id=$((vm_id + 1))
done
echo "使用可用的 VM ID: $vm_id"
...
echo "========================================="
echo "[4/10] 下載最新 Kali VM 映像檔 ..."
echo "========================================="
wget -c --show-progress "$kali_url"
echo "映像檔下載完成：$filename"

echo "========================================="
echo "[5/10] 解壓縮 Kali 映像檔 ..."
echo "========================================="
unar -f "$filename"
echo "解壓縮完成。"
...
if [ -z "$qcow2file" ]; then
  echo "找不到 qcow2 磁碟映像，請確認解壓縮內容！"
  exit 1
fi
echo "找到 qcow2 檔案：$qcow2file"

echo "========================================="
echo "[6/10] 建立 Kali VM ..."
echo "========================================="
if [ -z "$vlan_id" ]; then
...
echo "========================================="
echo "[9/10] 啟動 Kali VM ..."
echo "========================================="
qm start "$vm_id"
echo "Kali VM 已成功啟動。"

echo "========================================="
echo "[10/10] 完成所有作業。映像檔儲存於：$working_dir"
echo "========================================="
